...
int main(int argc, char **argv) {
    ...
    printf("Experiment 02: Parallel Scalability (Deferred)\n");
    printf("Grid, Threads, Ass4_Serial, Ass4_Parallel, Deferred_Serial, Deferred_Parallel\n");
    ...
    for (int g = 0; g < 3; g++) {
        ...
        initializepoints(points);
        double serial_def = 0.0;
        for (int i = 0; i < Maxiter; i++) {
            double t1 = omp_get_wtime();
            mover_serial_deferred(points, dx, dy);
            serial_def += omp_get_wtime() - t1;
        }
