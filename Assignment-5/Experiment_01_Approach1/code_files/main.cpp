...
int main(int argc, char **argv) {
    ...
    printf("Experiment 01: Serial Mover Execution Time Scaling (Deferred)\n");
    printf("Grid, Particles, Deferred_Total_Time\n");
    ...
    for (int g = 0; g < 3; g++) {
        ...
        for (int p = 0; p < 4; p++) {
            ...
            double deferred_time = 0.0;
            for (int iter = 0; iter < Maxiter; iter++) {
                clock_t start = clock();
                interpolation(mesh_value, points);
                mover_serial_deferred(points, dx, dy);
                clock_t end = clock();
                deferred_time += (double)(end - start) / CLOCKS_PER_SEC;
            }

            printf("%dx%d, %d, %lf\n", NX, NY, NUM_Points, deferred_time);
